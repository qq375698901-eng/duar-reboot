...
@export var disengage_range: float = 280.0
@export var attack_range_min: float = 88.0
@export var attack_range_max: float = 192.0
@export var attack_vertical_tolerance: float = 108.0
@export var retreat_range: float = 52.0
@export var attack_pause_duration_sec: float = 0.22
@export var move_accel: float = 240.0

@export_group("Projectile")
@export var projectile_scene: PackedScene
@export var projectile_hit_effect: String = "stun"
@export var projectile_stun_duration_sec: float = 0.20
@export var projectile_launch_height_px: float = 32.0
@export var projectile_launch_distance_px: float = 16.0
@export var projectile_gravity_force: float = 1960.0
@export var projectile_lifetime_sec: float = 1.35
@export var projectile_min_flight_time_sec: float = 0.36
@export var projectile_max_flight_time_sec: float = 0.72
@export var projectile_speed_hint: float = 220.0
@export var aim_height_offset: float = 6.0
@export var throw_horizontal_bias_px: float = 8.0
@export var throw_random_target_offset_px: float = 54.0
@export var muzzle_offset: Vector2 = Vector2(13.0, -15.0)
...
func _update_behavior(delta: float) -> void:
	...
	match _behavior_state:
		BehaviorState.IDLE:
			velocity.x = move_toward(velocity.x, 0.0, move_accel * delta)
			if target_detected and horizontal_distance < attack_range_min:
				_set_behavior_state(BehaviorState.REPOSITION)
			elif target_detected and target_in_attack_range:
		...
		BehaviorState.REPOSITION:
			...
			if target_in_attack_range:
				...
				velocity.x = move_toward(velocity.x, 0.0, move_accel * 2.0 * delta)
				return
			if horizontal_distance >= attack_range_min or desired_direction == 0.0:
				_set_behavior_state(BehaviorState.IDLE)
				velocity.x = move_toward(velocity.x, 0.0, move_accel * delta)
			else:
...
func get_stun_knockback_speed() -> float:
	if stun_base_duration_sec <= 0.0:
		return 0.0
	return stun_base_knockback_distance_px / stun_base_duration_sec
...
func _lock_attack_target() -> void:
	...
	if _has_valid_target():
		var random_offset_x := randf_range(-throw_random_target_offset_px, throw_random_target_offset_px)
		locked_target = _target.global_position + Vector2(random_offset_x + (float(facing) * throw_horizontal_bias_px), aim_height_offset)
...
func _spawn_projectile() -> void:
	...
	if projectile_instance.has_method("set"):
		projectile_instance.set("gravity_force", projectile_gravity_force)
		projectile_instance.set("lifetime_sec", projectile_lifetime_sec)
		projectile_instance.set("min_flight_time_sec", projectile_min_flight_time_sec)
		projectile_instance.set("max_flight_time_sec", projectile_max_flight_time_sec)
		projectile_instance.set("target_speed_hint", projectile_speed_hint)
		projectile_instance.set("fire_patch_duration_sec", fire_patch_duration_sec)
		projectile_instance.set("fire_patch_tick_interval_sec", fire_patch_tick_interval_sec)
		projectile_instance.set("fire_patch_damage_scale", fire_patch_damage_scale)
	if projectile_instance.has_method("launch_to"):
		projectile_instance.call("launch_to", _attack_locked_target_position, payload, self)


func _build_attack_payload() -> Dictionary:
	return {
		"attack_id": "dungeon_firebomber_firebomb",
		"damage": base_damage,
