...
const ATTACK_LUNGE_START_FRAME := 5
const ATTACK_LUNGE_END_FRAME := 6
const ATTACK_ACTIVE_FRAMES := {
	5: {
		"offset": Vector2(25.0, -17.0),
		"size": Vector2(32.0, 8.0),
	},
	6: {
		"offset": Vector2(28.0, -17.0),
		"size": Vector2(34.0, 8.0),
	},
}
...
@export var max_hp: float = 34.0
@export var base_damage: float = 10.0
@export var attack_interval_sec: float = 0.24
@export_group("")
@export var move_speed: float = 42.0
@export var hit_stun_horizontal_decel: float = 3000.0
@export var stun_base_duration_sec: float = 0.2
...
@export var attack_range: float = 162.0
@export var attack_vertical_tolerance: float = 96.0
@export var attack_pause_duration_sec: float = 0.12
@export var move_accel: float = 300.0

@export_group("Attack")
@export var attack_hit_effect: String = "stun"
@export var attack_stun_duration_sec: float = 0.26
@export var attack_launch_height_px: float = 56.0
@export var attack_launch_distance_px: float = 28.0
@export var attack_lunge_speed: float = 920.0
@export var attack_lunge_accel: float = 5200.0
@export var attack_lunge_decel: float = 5400.0
@export var attack_lunge_max_distance: float = 90.0
...
func _lock_attack_target() -> void:
	...
	if _has_valid_target():
		var to_target := _target.global_position - global_position
		var to_target_x := to_target.x
		if absf(to_target_x) > 1.0:
			direction = 1 if to_target_x > 0.0 else -1
			set_facing(direction)

		var desired_travel := maxf(0.0, absf(to_target_x) - attack_lunge_stop_distance)
		desired_travel = minf(desired_travel, attack_lunge_max_distance)
		var desired_vertical_travel := clampf(
			to_target.y,
			-attack_lunge_vertical_max_distance,
			attack_lunge_vertical_max_distance
		)
		_attack_locked_position = Vector2(
			global_position.x + float(direction) * desired_travel,
			global_position.y + desired_vertical_travel
		)
		_has_attack_lock = true
		return

	_attack_locked_position = Vector2(
		global_position.x + float(direction) * attack_lunge_max_distance,
		global_position.y
	)
	_has_attack_lock = true
...
func _update_attack_motion(delta: float) -> void:
	...
	if not _has_attack_lock:
		_attack_locked_position = global_position + Vector2(float(facing) * attack_lunge_max_distance, 0.0)
		_has_attack_lock = true

	var to_locked := _attack_locked_position - global_position
	if to_locked.length() <= attack_lunge_arrive_tolerance:
		_update_attack_recovery_motion(delta)
		return
	...
	if not _attack_burst_started:
		...
		var burst_velocity := to_locked.normalized() * attack_lunge_speed
		velocity = burst_velocity
		_attack_burst_started = true
		return

	velocity = velocity.move_toward(Vector2.ZERO, attack_lunge_decel * delta)
...
func _build_attack_payload() -> Dictionary:
	return {
		"attack_id": "dungeon_lancer_attack",
		"damage": base_damage,
