...
function DrawFireBottle($graphics, $throwHand, $support, $tip, [bool]$showFlame, $palette) {
    if (-not $showFlame) {
        return
    }

    $dir = NormalizeVec (SubVec $tip $throwHand)
    $normal = PerpRight $dir

    $bottleCenter = AddVec $throwHand (MulVec $dir 2.2)
    $bodyW = 5.6
    $bodyH = 7.2
    FillEllipse2D $graphics $palette.bottle_glass $palette.outline ($bottleCenter.X - ($bodyW / 2.0)) ($bottleCenter.Y - ($bodyH / 2.0)) $bodyW $bodyH

    $gripWrap = AddVec $throwHand (MulVec $dir 0.6)
    DrawCapsuleSegment $graphics $throwHand $gripWrap 1.8 $palette.strap $palette.outline

    $neckBase = AddVec $bottleCenter (MulVec $dir 2.0)
    $neckTop = AddVec $bottleCenter (MulVec $dir 3.8)
    DrawCapsuleSegment $graphics $neckBase $neckTop 2.0 $palette.bottle_neck $palette.outline

    $ragA = AddVec $neckTop (MulVec $normal 1.2)
    $ragB = AddVec $neckTop (MulVec $normal (-1.2))
    $ragTip = AddVec $neckTop (MulVec $dir 2.2)
    FillPolygon2D $graphics $palette.rag_main $palette.outline @($ragA, $ragTip, $ragB)

    FillEllipse2D $graphics $palette.bottle_highlight $palette.bottle_highlight ($bottleCenter.X - 1.3) ($bottleCenter.Y - 2.1) 1.4 3.0

    if ($showFlame) {
        $flameCore = AddVec $neckTop (MulVec $dir 3.0)
        $flameOuter = @(
            (AddVec $flameCore (V -1.2 1.3))
            (AddVec $flameCore (V 0.0 -2.6))
            (AddVec $flameCore (V 1.5 1.0))
            (AddVec $flameCore (V 0.0 2.1))
        )
        FillPolygon2D $graphics $palette.flame_outer $palette.outline $flameOuter

        $flameInner = @(
            (AddVec $flameCore (V -0.6 0.8))
            (AddVec $flameCore (V 0.0 -1.5))
            (AddVec $flameCore (V 0.8 0.7))
            (AddVec $flameCore (V 0.0 1.2))
        )
        FillPolygon2D $graphics $palette.flame_inner $palette.flame_inner $flameInner
    }
}
...
function DrawBody($graphics, $pose, $palette) {
    ...
    $bagCenter = AddVec $pose.pelvis (AddVec (MulVec $right -2.6) (MulVec $axis -0.4))
    FillEllipse2D $graphics $palette.bag_main $palette.outline ($bagCenter.X - 3.3) ($bagCenter.Y - 3.3) 6.6 6.6
    FillRect2D $graphics $palette.bag_flap ($bagCenter.X - 2.4) ($bagCenter.Y - 3.0) 4.8 2.2

    $bottleA = AddVec $pose.pelvis (AddVec (MulVec $right 2.6) (MulVec $axis 0.6))
    $bottleB = AddVec $pose.pelvis (AddVec (MulVec $right 3.8) (MulVec $axis -0.4))
    FillEllipse2D $graphics $palette.bottle_side $palette.outline ($bottleA.X - 1.5) ($bottleA.Y - 2.4) 3.0 4.8
    FillEllipse2D $graphics $palette.bottle_side $palette.outline ($bottleB.X - 1.5) ($bottleB.Y - 2.4) 3.0 4.8
    ...
    $scarfPoints = @(
        (V ($hoodX - 2.8) ($hoodY + 2.8))
        (V ($hoodX + 1.8) ($hoodY + 2.5))
        (V ($hoodX + 2.2) ($hoodY + 4.0))
        (V ($hoodX - 1.8) ($hoodY + 4.5))
    )
    FillPolygon2D $graphics $palette.scarf_main $palette.outline $scarfPoints
    ...
    DrawFireBottle $graphics $pose.front_hand $pose.back_hand $pose.weapon_tip $pose.flame $palette
}
...
$palette = @{
    ...
    hood_main = HexColor "#414039"
    face_shadow = HexColor "#181619"
    cloth_main = HexColor "#635e49"
    leather_main = HexColor "#5b4736"
    cape_main = HexColor "#4a4b42"
    bag_main = HexColor "#4a3d30"
    bag_flap = HexColor "#332920"
    pad_main = HexColor "#68634f"
    belt = HexColor "#372a24"
    accent_red = HexColor "#703133"
    scarf_main = HexColor "#70492f"
    arm_back = HexColor "#4f5144"
    arm_front = HexColor "#62634f"
    glove_back = HexColor "#756960"
    glove_front = HexColor "#85786e"
    leg_back = HexColor "#4c4e43"
    leg_front = HexColor "#615f55"
    boot_back = HexColor "#242125"
    boot_front = HexColor "#2e292d"
    bottle_glass = HexColor "#6c4b34"
    bottle_neck = HexColor "#7a5b43"
    bottle_side = HexColor "#5e412c"
    bottle_highlight = HexColor "#c8945b"
    rag_main = HexColor "#9d7248"
    strap = HexColor "#3b2c22"
    flame_outer = HexColor "#ef9b33"
    flame_inner = HexColor "#ffe08a"
    eye = HexColor "#e6edf4"
...
$animations = @(
    ...
    @{
        ...
        loop = $false
        frames = @(
            (Frame 24.0 22.5 22.0 33.0 27.0 16.0 29.6 28.2 -0.8 21.4 28.3 0.6 27.0 41.0 2.5 20.0 41.0 -1.8 39.4 28.1 $true $true)
            (Frame 23.3 23.2 21.4 33.8 26.4 16.8 28.0 27.2 -1.9 20.8 28.2 0.5 27.2 41.0 2.9 20.2 41.0 -1.8 33.4 25.8 $true $true)
            (Frame 22.1 24.0 20.4 34.6 25.0 18.0 25.2 24.6 -3.2 20.2 27.8 0.4 27.0 41.0 3.5 20.2 41.0 -2.1 29.0 20.6 $true $true)
            (Frame 21.8 24.4 20.1 35.0 24.7 18.3 23.7 22.3 -4.2 20.0 27.8 0.4 27.2 41.0 3.8 20.2 41.0 -2.2 26.6 16.7 $true $true)
            (Frame 22.8 23.0 20.9 34.2 25.1 17.2 31.2 22.7 -1.0 21.0 27.5 0.4 27.6 41.0 3.0 20.6 41.0 -1.8 39.6 18.0 $true $true)
            (Frame 24.8 22.1 22.4 33.4 27.2 16.2 34.6 24.4 0.2 22.2 28.0 0.3 28.4 41.0 2.1 21.2 41.0 -1.4 45.0 21.4 $true $false)
            (Frame 25.8 22.5 23.2 33.6 28.1 16.5 33.2 27.6 0.6 22.8 28.6 0.3 28.8 41.0 1.8 22.1 41.0 -1.1 43.0 26.8 $true $false)
            (Frame 24.5 22.7 22.3 33.2 27.2 16.3 30.8 28.0 -0.2 21.8 28.3 0.4 28.0 41.0 2.2 21.4 41.0 -1.4 39.8 28.5 $true $true)
        )
    }
    @{
        name = "hit"
...
$scenePath = Join-Path $sceneDir "dungeon_firebomber_sprite.tscn"
$manifestPath = Join-Path $artDir "dungeon_firebomber_manifest.json"
$previewPath = Join-Path $artDir "dungeon_firebomber_preview_4x.png"
...
foreach ($animation in $animations) {
    ...
    $fileName = "dungeon_firebomber_{0}_strip.png" -f $animation.name
    ...
    $sheetMeta += [PSCustomObject]@{
        name = $animation.name
        file = $fileName
        path = "res://art/enemies/dungeon_firebomber/$fileName"
        frames = $animation.frames.Count
        fps = $animation.fps
        loop = [bool]$animation.loop
    }
...
$manifest = [PSCustomObject]@{
    name = "dungeon_firebomber"
    display_name = "Dungeon Firebomber"
    ...
    notes = @(
        ...
        "fire-bottle silhouette with clear grab-lift-throw poses"
    )
...
$readmeLines = @(
    "# Dungeon Firebomber"
    ""
    "- Display name: Dungeon Firebomber"
    "- Frame size: 48x48"
    "- Background: transparent"
    "- Baseline: y = 41"
...
$spriteFramesBlock = @"
[sub_resource type="SpriteFrames" id="SpriteFrames_dungeon_firebomber"]
...
$($subLines -join "`r`n")
$spriteFramesBlock
[node name="DungeonFirebomberSprite" type="Node2D"]

[node name="AnimatedSprite2D" type="AnimatedSprite2D" parent="."]
texture_filter = 1
position = Vector2(0, 0)
sprite_frames = SubResource("SpriteFrames_dungeon_firebomber")
animation = &"idle"
frame = 0
centered = true
"@
